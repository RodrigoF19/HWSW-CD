...
typedef struct {
    ...
    XStream_matmul_uio_map maps[ MAX_UIO_MAPS ];
} XStream_matmul_uio_info;

/***************** Variable Definitions **************************************/
static XStream_matmul_uio_info uio_info;
...
static int uio_info_read_name(XStream_matmul_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XStream_matmul_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XStream_matmul_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XStream_matmul_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XStream_matmul_Initialize(XStream_matmul *InstancePtr, const char* InstanceName) {
	XStream_matmul_uio_info *InfoPtr = &uio_info;
